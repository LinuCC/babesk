...
class AdminGroupProcessing {
	function __construct () {
		...
		$this->groupInterface = new AdminGroupInterface();
		$this->msg = array(
			//NewGroup
			...
			'err_add_pc'			 => 'Konnte eine Preisklasse nicht hinzufügen. Möglicherweise ist der Gruppeneintrag in der Datenbank nun fehlerhaft!',
			...
			'err_del_pc'			 => 'konnte die zur Gruppe zugehörigen Preisklassen nicht löschen. Möglicherweise sind einige Datenbankeinträge nun fehlerhaft!',
		...
		if (isset($_POST['groupname'], $_POST['max_credit'])) {
			...
			foreach ($priceclasses as $priceclass) {
				try {
					...
					$this->groupInterface->ShowError('A Priceclass with the ID ' . $pc_id . ' could not be added: ' . $e->getMessage());
				}
			}
